---
title: "itsjon_FinalHomeworkCode_04"
...
# yz: The code is very clean and easy to read. I really like the use of the switch function. Since I used if else statement to determine which hypothesis test to use, but I think the switch function allowed the code to be more simpler and compact.
...
## yz: The code again is very simple and easy to use. I incorporated a lot of for loops, which made it look not as nice compared to yours. The only thing I have to comment is that the homework also asked for an interpretation of the point estimate of the slope and the outcome of the test. I think by including the interpretation will provide a more detailed explanation of the data analysis.
```
...
prediction_800 <- predict(lm2, newdata = new_data, interval = "prediction", level = 0.90)
...
predict_model <- data.frame(
  Model = c("Longevity ~ Brain Size", "Log(Longevity) ~ Log(Brain Size)"),
  Point_Estimate = c(pred[1, "fit"], prediction_800[1, "fit"]),
  PI_Lower = c(pred[1, "lwr"], prediction_800[1, "lwr"]),
  PI_Upper = c(pred[1, "upr"], prediction_800[1, "upr"]),
  R_Squared = c(r_squared1, r_squared2),
  PI_Range = c(pred[1, "upr"] - pred[1, "lwr"], prediction_800[1, "upr"] - prediction_800[1, "lwr"])
)
...
# There were many components to this homework that I had to figure out. 

# Part 1 was the most diffucult in my opinion, and that took longer compared to part 2. 

# There were a lot of questions for Part 2, and some I had a hard time figuring out how to put into neat plot lines. I was originally able to get the numbers for it at the very least. 

# I think this was the hardest Homework that I've worked on, in my opinion. 
```
